...
import { getGrade, getGradingPolicy, GetGradingPolicyResult, willBeGraded, WillGetCreditReason, WillTrackAttemptReason } from '../../utilities/grading-helper';
    ...
    async updateGrade(options: UpdateGradeOptions): Promise<UpdateResult<StudentGrade>> {
        return appSequelize.transaction(async (): Promise<UpdateResult<StudentGrade>> => {
            if (!_.isNil(options.updates.effectiveScore)) {
                try {
                    await StudentGradeOverride.create({
                        studentGradeId: options.where.id,
                        initiatingUserId: options.initiatingUserId,
                        newValue: options.updates.effectiveScore,
                    });    
                } catch (e) {
                    throw new WrappedError('Could not create grade override', e);
                }
            }
    
            if (!_.isNil(options.updates.locked)) {
                try {
                    await StudentGradeLockAction.create({
                        studentGradeId: options.where.id,
                        initiatingUserId: options.initiatingUserId,
                        newValue: options.updates.locked
                    });    
                } catch (e) {
                    throw new WrappedError('Could not create lock action', e);
                }
            }
    ...
    setGrade = async ({
        ...
        return appSequelize.transaction(async (): Promise<StudentWorkbook | undefined> => {
            if (gradeResult.gradingPolicy.willTrackAttemptReason === WillTrackAttemptReason.YES) {
                ...
                if (!gradeResult.gradingPolicy.isLate) {
                    studentGrade.numLegalAttempts++;
                }
                if (!gradeResult.gradingPolicy.isExpired) {
